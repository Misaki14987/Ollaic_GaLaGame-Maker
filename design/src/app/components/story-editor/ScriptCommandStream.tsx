...
function InsertZone({
  ...
  return (
    ...
    >
      ...
      {open && (
        <div className="absolute left-1/2 top-5 z-50 w-72 -translate-x-1/2 rounded border border-border bg-surface-container-high p-3 shadow-lg">
...
function ScriptCommandCard({
  ...
  drag(drop(ref));
  const canCreateUnlock =
    onCreateUnlockNode &&
    ((node.type === 'changeBg' &&
      (node.asset || node.content || '').trim() &&
      (node.asset || node.content || '').trim() !== 'none') ||
      (node.type === 'bgm' &&
        (node.asset || node.content || '').trim() &&
        (node.asset || node.content || '').trim() !== 'none'));
      ...
      <ContextMenu>
        <ContextMenuTrigger asChild>
          ...
          >
            ...
            >
              <div
                className={`absolute right-0 top-0 px-2 py-1 text-[9px] uppercase tracking-tight ${tagClass}`}
              >
                {tag}
              </div>
              ...
              {isBackground ? (
                <div className="flex items-center gap-4">
                  <div className="flex h-16 w-24 shrink-0 items-center justify-center overflow-hidden border border-outline-variant/20 bg-surface-container-highest">
                    {projectPath && (node.asset || node.content) ? (
                      <img
                        src={convertFileSrc(
                          `${projectPath}/game/background/${node.asset || node.content}`,
                        )}
                        alt=""
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="mb-1 font-bold text-foreground">
                      设置背景: {node.asset || node.content || '未选择背景'}
                    </p>
                  </div>
                </div>
                ...
                <>
                  <div className="mb-4 flex items-center gap-2">
                    ...
                    <span className="font-bold text-tertiary">
                      抉择分支: {node.content || '剧情选择'}
                    </span>
                  </div>
                  <div className="space-y-2">
                    {(node.choices?.length
                      ? node.choices
                      : [{ text: getCommandSummary(node), target: '@next' }]
                    ).map((choice, choiceIndex) => (
                      <div
                        key={`${node.id}-${choiceIndex}`}
                        className="flex items-center justify-between border border-tertiary/20 bg-surface-bright p-2 text-sm"
                      >
                        <span>
                ...
                <div className="flex items-center gap-4">
                  <div className="flex h-20 w-14 shrink-0 items-center justify-center overflow-hidden rounded">
                    {projectPath &&
                    (node.asset || node.content) &&
                    (node.asset || node.content) !== 'none' ? (
                      <img
                        src={convertFileSrc(
                          `${projectPath}/game/figure/${node.asset || node.content}`,
                        )}
                        alt=""
                  ...
                  <span className="min-w-0 truncate text-sm">{figureLabel(node, characters)}</span>
                </div>
              ) : (
            ...
            <div className="absolute right-2 top-2 z-10 flex items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-44">
                  ...
                  {onPasteNode && (
                    <DropdownMenuItem onClick={() => onPasteNode(index)} disabled={!clipboardNode}>
        ...
        <ContextMenuContent className="w-40">
          ...
          {canCreateUnlock && (
            <>
              <ContextMenuSeparator />
              <ContextMenuItem
                onClick={() => onCreateUnlockNode?.(node, index + 1)}
                className="gap-2 text-xs"
              >
...
export function ScriptCommandStream({
  ...
  const areaPasteIndex = useCallback(
    (container: HTMLElement, clientY: number): number => {
    ...
    <section className="relative flex min-w-0 flex-1 flex-col bg-background">
      ...
      <div className="relative z-10 flex h-10 shrink-0 items-center justify-between border-b border-outline-variant/20 bg-surface-bright/50 px-4">
        <div className="flex min-w-0 items-center gap-4">
          <span className="shrink-0 text-xs font-bold tracking-widest text-on-surface-variant">
            指令流编辑
          </span>
          ...
          <div className="mx-auto max-w-4xl space-y-6 pb-20">
            {nodes.length === 0 ? (
              <div className="flex min-h-[360px] flex-col items-center justify-center gap-3 border border-dashed border-outline-variant/50 bg-surface-bright p-8 text-center text-muted-foreground">
                ...
                <button
                  type="button"
                  onClick={() => onInsertNode('dialogue', 0)}
                  className="bg-primary px-4 py-2 text-sm font-semibold text-on-primary story-os-chamfer-tr"
                >
                  添加第一句对白
                </button>
              </div>
              ...
              visibleNodes.map(({ node, index }, visibleIndex) => {
                ...
                const displayCharacter = isDialogue
                  ? ownChar || effectiveSpeakers.get(node.id) || ''
                  : '';
                const characterInherited = isDialogue && !ownChar && !!displayCharacter;
                const charColor =
                  isDialogue && displayCharacter && characterColors?.[displayCharacter]
                ...
                return (
                  <Fragment key={node.id}>
                    ...
                    {onInsertNode && <InsertZone atIndex={index} onInsert={onInsertNode} />}
                    <ScriptCommandCard
                      node={node}
                      index={index}

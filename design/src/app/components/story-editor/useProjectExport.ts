...
import type { ExportTaskState } from '../ProjectMetadataDialog';
...
export function useProjectExport({ projectPath, ensureSaved }: ProjectExportOptions) {
  ...
  const saveMetadata = useCallback(async (next: ProjectMetadata) => {
  ...
  const exportWithMetadata = useCallback(async (
    ...
    try {
      ...
      setTask({ status: 'savingMetadata', warnings: [], issues: [], failureCount });
      await saveProjectMetadata(projectPath, payload);
      setMetadata(payload);
      setTask({ status: 'exporting', warnings: [], issues: [], failureCount });
      const result = await exportProject(projectPath, outputDir, asZip, payload);
      setTask(result.success
        ? {
            status: 'succeeded',

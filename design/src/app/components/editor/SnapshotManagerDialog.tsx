...
export function SnapshotManagerDialog({
  ...
  const selected = useMemo(
    () =>
      filteredSnapshots.find((snapshot) => snapshot.id === selectedId) ??
      filteredSnapshots[0] ??
      null,
            ...
            {selected ? (
              <div className="space-y-4 text-sm">
                ...
                {selected.description && (
                  <SnapshotDetail label="说明" value={selected.description} />
                )}
                <SnapshotDetail
                  label="文件数量"
                  value={selected.fileCount != null ? String(selected.fileCount) : '旧快照未记录'}
                />
                <SnapshotDetail
                  label="元信息"
                  value={
                    selected.metadataIncluded === true
                      ? '包含'
                      : selected.metadataIncluded === false
                        ? '不包含'
                        : '旧快照未记录'
                  }
                />
                <SnapshotDetail
                  label="编辑器状态"
                  value={selected.includesEditorState ? '包含' : '未记录或不包含'}
                />
                ...
                <div className="rounded-md border border-border bg-background/50 p-3 text-xs leading-5 text-muted-foreground">
                  回滚会先自动创建 before-restore 备份，然后恢复此快照里的
                  game、项目元信息和编辑器状态。
                </div>
              </div>
            ) : (
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
            ...
            {snapshot.metadataIncluded && (
              <span className="rounded bg-secondary px-1.5 py-0.5">元信息</span>
            )}
            {snapshot.includesEditorState && (
              <span className="rounded bg-secondary px-1.5 py-0.5">编辑器状态</span>
            )}
          </div>
        </div>
...
function SnapshotDetail({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div>
      <div className="text-xs uppercase tracking-widest text-muted-foreground font-mono-family">
        {label}

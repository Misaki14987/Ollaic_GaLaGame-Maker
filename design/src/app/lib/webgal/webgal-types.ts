...
const commandToCategory: Record<WebGalCommandType, CommandCategory> = (() => {
  ...
  (Object.entries(commandCategories) as [CommandCategory, readonly WebGalCommandType[]][]).forEach(
    ([cat, types]) =>
      types.forEach((t) => {
        map[t] = cat;
      }),
...
export const typeBorderClass: Record<WebGalCommandType, string> = Object.fromEntries(
  Object.entries(commandToCategory).map(([type, cat]) => [type, categoryBorderClass[cat]]),

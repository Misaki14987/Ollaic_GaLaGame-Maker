...
describe('resolveSpriteFile', () => {
  ...
  it('resolves a variant sprite (empty file) by emotion prefix to a qualified path', () => {
    ...
    expect(resolveSpriteFile(SHIZUKA, SHIZUKA.sprites[1], assets)).toBe(
      'char_shizuka/静香_生气_1700000000.png',
    );
  });
  ...
  it('picks the newest variant image when several match the prefix', () => {
    ...
    expect(resolveSpriteFile(SHIZUKA, SHIZUKA.sprites[1], assets)).toBe(
      'char_shizuka/静香_生气_1700000009.png',
    );
  });
...
describe('resolveFigureByEmotion', () => {
  ...
  it('matches a character by id and by alias', () => {
    expect(resolveFigureByEmotion([SHIZUKA], 'char_shizuka', '默认', assets)?.file).toBe(
      'shizuka_default.png',
    );
    expect(resolveFigureByEmotion([SHIZUKA], '小静', '默认', assets)?.file).toBe(
      'shizuka_default.png',
    );
  });

  it('is case-insensitive on emotion', () => {
    expect(resolveFigureByEmotion([SHIZUKA], '静香', '默认', assets)?.file).toBe(
      'shizuka_default.png',
    );
  });
